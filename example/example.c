...
static void init_resources()
{
    ...
    PS_NAME("font1", PS_ASSERT_RES(ps_res_add_ttf(example_OpenSans_Medium_ttf, example_OpenSans_Medium_ttf_sz)));
    PS_NAME("font2", PS_ASSERT_RES(ps_res_add_ttf(example_Born2bSportyFS_otf, example_Born2bSportyFS_otf_sz)));

    PS_NAME("music", PS_ASSERT_RES(ps_res_add_music(example_nemesis_mod, example_nemesis_mod_sz)));

    PS_NAME("sound", PS_ASSERT_RES(ps_red_add_sound(example_shotgun_wav, example_shotgun_wav_sz)));
}
...
static void update(size_t timestep_us)
{
    ...
    while (SDL_PollEvent(&e)) {
        ...
        if (e.type == SDL_EVENT_KEY_DOWN && e.key.key == SDLK_SPACE)
            PS_ASSERT(ps_audio_play_sound(PS_IDX("sound")));
    }
}
...
static void create_scene(ps_Scene* scene)
{
    ...
    PS_ASSERT(ps_scene_push_context(scene, ps_create_context_with(CTX_ZOOM, 2.f, NULL)));

    PS_ASSERT(ps_scene_add_image_with(scene, PS_IDX("happy"), (SDL_Rect) { 100, 100, 58, 78 },
        CTX_ROTATION, 90.f, CTX_OPACITY, .5f, NULL));

    PS_ASSERT(ps_scene_add_text_with(scene, PS_IDX("font1"), "Hello world to ALL!", POS(10, 10), 32, (SDL_Color) { 0, 0, 0, 255 },
        CTX_ZOOM, .5f, NULL));

    PS_ASSERT(ps_scene_add_text(scene, PS_IDX("font2"), "Press SPACE to fire shotgun", POS(10, 30), 18, (SDL_Color) { 0, 0, 0, 255 }, NULL));
}
...
int main()
{
    ...
    PS_ASSERT(ps_graphics_set_bg(160, 180, 200));

    init_resources();

    PS_ASSERT(ps_audio_choose_music(PS_IDX("music")));
    PS_ASSERT(ps_audio_play_music(true));

    while (ps_graphics_running()) {
        update(ps_graphics_timestep_us());
        PS_ASSERT(ps_audio_step());

        ps_Scene scenes[1];
        create_scene(&scenes[0]);
        PS_ASSERT(ps_graphics_render_scenes(scenes, 1));

        post_scene();
        PS_ASSERT(ps_graphics_present());
    }
